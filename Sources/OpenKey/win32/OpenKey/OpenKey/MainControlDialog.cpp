...
void MainControlDialog::onUpdateButton() {
    ...
    if (OpenKeyManager::checkUpdate(newVersion)) {
        WCHAR msg[256];
        wsprintf(msg,
            TEXT("NiceKey có phiên bản mới (%s), bạn có muốn cập nhật không?"),
        ...
        MessageBox(hDlg, _T("Bạn đang dùng phiên bản mới nhất!"), _T("NiceKey Update"), MB_OK);
    }
...
void MainControlDialog::requestRestartAsAdmin() {
    ...
    if (vRunAsAdmin && !IsUserAnAdmin()) {
        int msgboxID = MessageBox(
            hDlg,
            _T("Bạn cần phải khởi động lại NiceKey để kích hoạt chế độ Admin!\nBạn có muốn khởi động lại NiceKey không?"),
            _T("NiceKey"),
            MB_ICONEXCLAMATION | MB_YESNO
        );
        if (msgboxID == IDYES) {
